...
export class ParameterValidator {
     ...
     * 验证交易对参数
     */
    static validateSymbol(symbol, required = true) {
        if (!symbol) {
            if (required) {
                return {
                    valid: false,
     ...
     * 验证数量参数
     */
    static validateQuantity(quantity, required = true) {
     ...
     * 验证交易方向
     */
    static validateSide(side, required = true) {
        if (!side) {
            if (required) {
                return {
                    valid: false,
     ...
     * 综合参数验证
     */
    static validateParameters(params, rules) {
        const errors = [];
        const warnings = [];
        const suggestions = [];
        const validatedData = {};
     ...
     * 格式化验证错误消息
     */
    static formatValidationError(result) {
        let message = `❌ 参数验证失败:\n\n${result.error}`;
     ...
     * 格式化验证警告消息
     */
    static formatValidationWarnings(warnings) {
