...
export class ResultFormatter {
     ...
     * 格式化数量显示
     */
    static formatQuantity(quantity, asset = '') {
     ...
     * 格式化现货下单成功结果
     */
    static formatSpotOrderSuccess(data) {
...
建议关注市场价格变动，及时调整策略`;
     ...
     * 格式化合约下单成功结果
     */
    static formatFuturesOrderSuccess(data) {
        const symbol = data.symbol || '';
...
关注保证金比例，避免被强制平仓`;
     ...
     * 格式化余额查询结果
     */
    static formatSpotBalances(balances) {
...
可使用价格查询工具计算总资产USD估值。`;
     ...
     * 格式化持仓查询结果
     */
    static formatFuturesPositions(positions) {
...
${this.getPositionAdvice(totalPnl, activePositions)}`;
     ...
     * 获取风险等级
     */
    static getRiskLevel(pnl, notional) {
     ...
     * 获取持仓风险等级
     */
    static getPositionRiskLevel(liquidationPrice, markPrice, isLong) {
     ...
     * 获取持仓操作建议
     */
    static getPositionAdvice(totalPnl, positions) {
        if (totalPnl > 0) {
     ...
     * 格式化24小时行情
     */
    static format24hrTicker(ticker) {
     ...
     * 格式化账户信息
     */
    static formatAccountInfo(account) {
        ...
        if (account.balances && account.balances.length > 0) {
            ...
            // 显示前5个有余额的币种
            activeBalances.slice(0, 5).forEach((balance) => {
                const emoji = this.getCurrencyEmoji(balance.asset);
